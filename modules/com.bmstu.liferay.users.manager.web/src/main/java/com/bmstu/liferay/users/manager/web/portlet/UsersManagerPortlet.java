...
public class UsersManagerPortlet extends MVCPortlet {
	 ...
	 * 
	 * @param request - request. Can't be <code>null</code>.
	 * @param response - response. Can't be <code>null</code>
	 * 
	...
	public void addTraccarUser(ActionRequest request, ActionResponse response) throws PortalException {
		...
		if (traccarUserId > 0) {
			try {
				traccarUserLocalService.updateTraccarUser(serviceContext.getUserId(), traccarUserId, traccarUserName,
						traccarUserPassword, traccarUserEmail, serviceContext);
				SessionMessages.add(request, "Traccar User Updated");
			} catch (Exception e) {
	...
	public void deleteTraccarUser(ActionRequest request, ActionResponse response) throws PortalException {
		...
		try {
			traccarUserLocalService.deleteTraccarUser(traccarUserId, serviceContext);
		} catch (Exception e) {
			Logger.getLogger(UsersManagerPortlet.class.getName()).log(Level.SEVERE, null, e);
		}
	}
